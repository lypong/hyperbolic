...
// Crée le "modèle" qui permet de transmettre nos variables dans les
// différentes phases de Nannou.
fn model(app: &App) -> Model {
    // On créé une fenêtre à laquelle on attache une instance d'objet
    // représentant une interface graphique interactive.
    app.new_window()
        .raw_event(raw_event)
        .view(view)
        .build()
    ...
    let mut tiling =
        Tiling::new(settings.p, settings.q, settings.max_depth);
    tiling.compute();
    Model {
        tiling,
        settings,
...
fn update(_app: &App, model: &mut Model, _update: Update) {
    ...
    // On crée nos curseurs qui permettent à l'utilisateur de fournir
    // un input.
    egui::Window::new("Paramètres").show(&ctx, |ui| {
        changed |= ui
            .add(
                egui::Slider::new(&mut model.settings.p, 3..=MAX_P)
                    .text("Nombre de côtés"),
            )
            .changed();
        changed |= ui
            .add(
                egui::Slider::new(&mut model.settings.q, 3..=MAX_Q)
                    .text("Polygones rencontrés en un point"),
            )
            .changed();
        // Permets une valeur de profondeur de récursion raisonnable
        // et adaptée à p, afin d'éviter les ralentissements.
        ...
        model.settings.max_depth =
            model.settings.max_depth.clamp(1, max_depth_permitted);
        changed |= ui
            .add(
                egui::Slider::new(
                    &mut model.settings.max_depth,
                    1..=max_depth_permitted,
                )
                .text("Profondeur maximale"),
            )
            .changed();
    });
